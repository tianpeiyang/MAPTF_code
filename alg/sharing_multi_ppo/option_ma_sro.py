...
class SREmbedding:
    def __init__(self, option_dim, agent_num, n_features, args, sess, logger, name,
        ...
        self.n_features = n_features + agent_num
        self.logger = logger
        ...
        self.q_sr_indiv = tf.reduce_sum(self.q_sr_raw, axis=-1, keepdims=True)

        with tf.variable_scope(self.name + '_embedding_loss'):
            self.recon_loss = tf.reduce_mean(tf.square(self.s - self.recon))
            corresponding_imed_r = tf.reduce_sum(self.imed_r, axis=-1)
    ...
    def get_embedding(self, s, agent_id=0):
        s = np.array(s).reshape(-1, self.n_features - self.agent_num)
    ...
    def get_agent_obs(self, obs, agent_id=0):
        ...
        agent_one_hot = np.eye(self.agent_num)[agent_id_arr]
...
class SROption:
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            with tf.variable_scope("option_sr"):
                ...
                sr_omega = tf.layers.dense(l_a_2, self.option_dim * self.emb_dim, None, trainable=trainable,
                                           kernel_initializer=self.w_init, name='omega_sr')
    ...
    def choose_o(self, s, agent_id):
        if np.random.uniform() < self.epsilon:
            emb = self.sr_emb.get_embedding(s, agent_id)
            emb = emb[np.newaxis, :]
            sr = self.sess.run(self.sr_eval_list[agent_id], feed_dict={self.emb: emb})
            _, q = self.sr_emb.get_Q_sr(np.reshape(sr, (-1, self.emb_dim)))
    ...
    def get_t(self, s_, option, agent_id):
        emb_ = self.sr_emb.get_embedding(s_, agent_id)
    ...
    #     emb = emb[np.newaxis, :]
    #     emb = self.get_agent_obs(emb, agent_id)
    ...
    def update(self, observation, option, reward, done, observation_, actor, agent_id):
        ...
        if not done:
            ...
            emb_ = self.sr_emb.get_embedding(observation_, agent_id)
            # size = [1, option_dim * emb_dim]
            ...
            _, q_sr_next = self.sr_emb.get_Q_sr(np.reshape(sr_next, (-1, self.emb_dim)))
        ...
        emb_batch = self.sr_emb.get_embedding(state_batch, list(agent_id_batch))
        next_emb_batch = self.sr_emb.get_embedding(next_state_batch, list(agent_id_batch))
        # size = [-1, option_dim, emb_dim]
        ...
        _, q_sr_next_eval = self.sr_emb.get_Q_sr(np.reshape(sr_next_eval, (-1, self.emb_dim)))
        ...
        # FIXME 1104 - update embedding and reward weights with (s,r,agent_id)
        self.sr_emb.update(state_batch, reward_batch, list(agent_id_batch))
