...
class SoftCategoricalPd(Pd):
    ...
    def cross_entropy(self, other):
        return self.kl(other) + self.entropy()
    def soft_max_sample(self, other, term):
        u1 = tf.random_uniform(tf.shape(self.logits))
        y1 = U.softmax(self.logits - tf.log(-tf.log(u1)), axis=-1)
        u2 = tf.random_uniform(tf.shape(other.logits))
        y2 = U.softmax(other.logits - tf.log(-tf.log(u2)), axis=-1)
        logits = y1 + term * y2
        logits_out = logits / U.sum(logits)
        return self.gumbel_sample(logits=logits_out)
    def params(self):
        return self.logits
    def sample(self):
        #u = tf.random_uniform(tf.shape(self.logits))
    ...
    def gumbel_sample(self, logits=None, temperature=0.1, hard=True):
        if logits is None:
            logits = self.logits
        u = tf.random_uniform(tf.shape(logits))
        y = U.softmax((logits - tf.log(-tf.log(u))) / temperature, axis=-1)
        if hard:
            k = tf.shape(logits)[-1]
