...
class PPO:
    def __init__(self, n_actions, n_features, args, SESS, logger, index):
        self.index = index
        self.args = args
        ...
        self.o_act_probs, self.o_policy_param = self.build_actor_net(self.args['old_policy'] + "_" + str(self.index))
    ...
    def build_loss(self):
        ...
        with tf.variable_scope('update_actor' + "_" + str(self.index)):
            ...
            with tf.variable_scope('loss/clip' + "_" + str(self.index)):
    ...
    def update(self, actor, s, a, r, options, terms, epi, agentid):
        ...
        for i, o in enumerate(options):
            ...
            if self.args['continuous_action']:
                ...
                if o == agentid:
                    a_prob = actor[o].choose_hold_action(s[i])
                else:
                    a_prob = actor[o].choose_acton_prob(s[i], a[i])
        ...
        if self.args['continuous_action']:
            for i in range(self.args['epi_train_times']):
                ...
                self.logger.write_tb_log('entropyTS_' + str(self.index), entropyTS, self.learning_step)
