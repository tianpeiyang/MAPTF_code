...
class PPO:
    def __init__(self, n_actions, n_features, args, SESS, logger, i):
        ...
        self.obs = tf.placeholder(tf.float32, [None, n_features], 's')

        self.act_probs, self.policy_param = self.build_actor_net(self.args['policy'] + "_" + str(self.index))
        self.o_act_probs, self.o_policy_param = self.build_actor_net(self.args['old_policy'] + "_" + str(self.index), trainable=False)
        self.v_preds, self.v_param = self.build_critic_net('critic' + "_" + str(self.index))
        ...
        with tf.variable_scope('train_inp' + "_" + str(self.index)):
            if self.args['continuous_action']:
    ...
    def build_loss(self):
        with tf.variable_scope('update_critic' + "_" + str(self.index)):
        ...
        with tf.variable_scope('update_actor' + "_" + str(self.index)):
    ...
    def get_action_and_v(self, s):
        ...
        if self.args['continuous_action']:
            ...
            if self.args['stochastic']:
                actions, v_preds, p = self.sess.run([self.act_probs, self.v_preds, self.act_probs], feed_dict={self.obs: obs})
                return actions, v_preds
    ...
    def update(self, actor, s, a, r, options, terms, epi, agentid=0):
        ...
        for i in range(self.args['epi_train_times']):
            ...
            self.logger.write_tb_log('a_loss_' + str(self.index), a_loss, self.learning_step)
            self.logger.write_tb_log('c_loss_' + str(self.index), c_loss, self.learning_step)
            self.logger.write_tb_log('clip_' + str(self.index), clip, self.learning_step)
            self.logger.write_tb_log('entropy_' + str(self.index), entropy, self.learning_step)
